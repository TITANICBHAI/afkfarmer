...
The workspace Chromium process can launch Patchright only when the GCC-provided
...
**Why:** Patchright's Python `greenlet` extension could not load
...
**How to apply:** Use the PC runtime wrapper for local browser startup and for
tests that import the browser adapter. Treat a provider block as a
failed/manual-takeover state; do not bypass Cloudflare or claim mailbox
verification without an observed address and message.